...
class PositionalEncoding(nn.Module):
  def __init__(
    self, d_model: int, dropout_rate: float = 0.1, max_len: int = 256
  ):
    super().__init__()
...
class TransformerDecoderLayer(nn.Module):
  ...
  def forward(  # pylint: disable=arguments-renamed
    ...
    if self.pre_ln:
      ...
      sa_out, cache = self._sa_block(
        x, tgt_mask, decode=decode, max_len=max_len, cache=cache, index=index
      )
  ...
  def _mha_block(
    self, x: Tensor, mem: Tensor, attn_mask: Optional[Tensor]
  ) -> Tensor:
